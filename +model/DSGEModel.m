...
% abstract superclass for model
    ...
    methods
        ...
        function [fx,Jac,V]=solveAtPoint(obj,solvec,point,mode,trans,expStruct,vals)
            % transition
            [nextst,outstr]=obj.calcStateTransition(point,solvec,0,trans,vals);
            % equations
        ...
        function [resmat,VF,TF,failedPointsIndex]=solvePointList(mobj,tmp_grid,tmp_resmat,tmp_transmat,...
...
%           for i=1:gr_npt
                ...
                succ='|';
                gvec=tmp_resmat(i,:)';
                p0=exp(gvec(4)); % current guess for house price
                
                % compute expectations
                transpts = [(1:mobj.Exogenv.exnpt)',reshape(trans,mobj.Exogenv.exnpt,mobj.NSTEN)];
                expStruct = computeExpectations(mobj, point(1), transpts, p0, vtrans);      
                vals=mobj.evaluateVal(point);
                
                % prepare function handle
                fh_solveAtPoint=@(sol)solveAtPoint(mobj,sol,point,0,trans,expStruct,vals);
                ...
                [~,~,V]=solveAtPoint(mobj,xr,point,1,trans,expStruct,vals);
        ...
        % solve model at list of points
        function [resmat_failed,VF_failed,TF_failed,n_succ]=solvePointListFailed(mobj,grid,index_failed,resmat,...
